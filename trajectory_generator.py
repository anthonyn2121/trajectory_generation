...
class PolyTraj(object):
    def __init__(self, r, waypoints):
        ...
        ## this is an arbitrary value, not definitive
        ...
        self.times = np.append(
            np.array([[0]]), self.times
        )  ## Insert 0 at the beginning of self.times
        self.times /= 2
        ...
        for r in range(1, self.r):
            ...
            for i in range(self.waypoints.shape[0] - 1):
                if r == 1:
                    drdt = (self.waypoints[i + 1] - self.waypoints[i]) / self.times[
                        i + 1
                    ]
                    dr_array[i + 1] = drdt
                else:
    ...
    def __polynomial_basis(self, t, order=0):
        ...
        temp = np.array(
            [t**i for i in range(self.num_coefficients - order - 1, -1, -1)]
        )
        basis[: len(temp)] = temp
        return basis
    ...
    def __basis_coefficients(self, order=0):
        ...
        return np.append(
            coefficients[::-1], np.zeros(self.num_coefficients - len(coefficients))
        )
    ...
    def __set_constraints(self, T, start, end):
        ...
        for j in range((self.num_coefficients - 2) // 2):
            order = j + 1
            A[row, -order - 1] = np.polyder(
                np.poly1d([1] * self.num_coefficients), order
            )(0)
            A[row + 1, :] = self.__basis_coefficients(order) * self.__polynomial_basis(
                T, order
            )
            row += 2
    ...
    def __get_flat_output(self, t, coeffs, axis):
        for i in range(len(self.d) + 1):
            ...
            self.flat_output[key][axis] = (
                self.__basis_coefficients(order=i) * self.__polynomial_basis(t, order=i)
            ) @ coeffs
